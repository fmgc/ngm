...
public class MiningEnvironment extends TimeSteppedEnvironment {
    ...
    public void init(String[] args) {
        ...
        if (args.length > 6) 
            numberOfSteps = Integer.parseInt(args[6]);
    ...
    public boolean executeAction(String ag, Structure action) {
        ...
        try {
            ...
            if (!action.equals(drop) && random.nextDouble() < model.getAgFatigue(agId)) {
                logger.finer("Action "+action+" from agent "+ag+" failed!");
    ...
    public void initWorld(int w) {
        ...
        try {
            ...
            int msteps = numberOfSteps == -1 ? model.getMaxSteps() : numberOfSteps;
            if (msteps == 0) msteps = 100000;
            model.setMaxSteps(msteps);
    ...
    protected void stepFinished(int step, long time, boolean timeout) {
        ...
        sum += time;
        if (step % 10 == 0) logger.info("Cycle "+step+" finished in "+time+" ms, mean is "+(sum/step)+".");
