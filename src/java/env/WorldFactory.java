...
public class WorldFactory  {
    ...
    public static WorldModel worldFromContest2007(String simId) throws Exception {
        File f = new File("serverconfig.contest-2007.xml");
        if (! f.exists()) {
            f = new File("applications/jason-team/"+f);
            if (! f.exists()) {
                System.out.println("Server config file not found!");
                return null;
            }
        }
        ...
        if (fence == null) {
            System.out.println("Didn't find fence configuration in the conf file!");
            return null;
        }
